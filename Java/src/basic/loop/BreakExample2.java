...
public class BreakExample2 {
	public static void main(String[] args) {
		...
		/*
		  # 무한 루프
	         ...
	         - 처음 반복문을 구성할 때, 개발자가 사전에 정확한 반복 횟수를
	          파악할 수 없다면, 무한루프를 일단 구성해 두고 특정 조건을 통해 종료할 수 있도록 코드를 설계합니다.
	         - 일반적으로 정확한 반복 횟수를 알고 있다면 for문을 사용하고,
	          정확한 반복 횟수를 모른다면 while을 통한 무한 루프를 형성하여
	          사용합니다. while(true)-> 논리상수임. 무조건 실행되기 때문에 break를 통해서만 
	          종료할 수 있다. , for(;;)
	         */
		...
		while(true) {
			...
			System.out.println("정답을 모르시면 0을 입력허묜 종료됩니다");
			System.out.print("> ");
			int answer = sc.nextInt();
			
			if(answer == 90) {
				System.out.println("정답입니다.");
				break; //정답을 맞췄을 경우에만 끝나도록 설정
			}else if(answer == 0) {
